...
class distributed_solution:
    def __init__(self, VPP_data_file, prosumer_data_file):
        self.VPP_data_file = VPP_data_file
        self.prosumer_data_file = prosumer_data_file
    # 其他东西都与分布式算法保持一致，这里只是测试加入标准差为1的高斯噪声的影响

    def iteration_process(self):
        prosumer_data_file = self.prosumer_data_file
        VPP_data_file = self.VPP_data_file
        ...
        mu_para = np.zeros(24)
        # 需要对solution进行初始化
        prosumers_pre_solution = 1.01*np.load('data/prosumers_net_output.npy')
        ...
        while k < ite_num:
            ...
            print('all_net_power_shape:', all_net_power.shape)
        ...
        # plt.show()
        # 这里需要以每一次迭代的终止值作为求解OPT gap的随机变量，并且对这个随机变量的分布进行描述
        # 保存目标函数的最终迭代值
        final_obj = values[-1]
        # 保存每个prosumer的最终输出以及aggregator的输出，以判断是否满足耦合约束
        final_prosumer_output = k_prosumer_net_power_save
        final_aggregator_output = VPP_solution
        # 用这三个变量作为输出
        return OPT_value, final_obj, final_prosumer_output, final_aggregator_output
        # 在主函数中迭代100次左右，计算该随机变量的均值和方差
        # 测试该class是否可以顺利运行，对输出的描述进行测试，先print结果